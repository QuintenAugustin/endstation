...
List<Card> createDeck() {
  ...
  List<int> ranks = List.generate(13, (index) => index + 2); // Generiere Ränge von 2 bis 14 (Ace)
  List<Card> deck = [];
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      themeMode: ThemeMode.system, // Benutzt Systemeinstellungen für den Theme-Modus
      home: HomePage(),
    );
  }
}
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 50), // Steuert die Breite der Buttons
              child: ElevatedButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => GamePage(spielerzahl: spielerzahl)),
                  );
                },
                child: Text('Spielen'),
              ),
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 100), // Steuert die Breite der Buttons
              child: ElevatedButton(
                onPressed: () {
                  Navigator.push(
                    context,
...
class _GamePageState extends State<GamePage> {
  ...
  void drawCards() {
    ...
    selectedCard = hand[random.nextInt(hand.length)]; // Wähle zufällig eine Karte aus den gezogenen Karten
  }
    ...
    if (playerGuesses.length < widget.spielerzahl) {
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Spieler ${playerGuesses.length + 1}, ist die Karte rot oder schwarz?'),
            actions: <Widget>[
              TextButton(
                onPressed: () {
  ...
  void revealCardAndShowResults() {
    ...
    bool isCardRed = selectedCard!.suit == 'hearts' || selectedCard!.suit == 'diamonds';
    List<bool> results = playerGuesses.map((guess) => guess == isCardRed).toList();
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: results.asMap().entries.map((entry) {
                ...
                return Text('Spieler ${idx + 1}: ${result ? "Richtig" : "Falsch"}');
              }).toList(),
            ),
          ),
          actions: <Widget>[
